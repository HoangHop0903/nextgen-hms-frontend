  ...
  return (
    <div>
      ...
      {activeSubTab === "medicines" ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {prices.map(item => (
            <div key={item.MaBangGia} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-sm shadow-sm p-4 flex flex-col group hover:border-indigo-500 dark:hover:border-indigo-400 hover:shadow-md transition-all">
              <div className="flex justify-between items-start mb-3">
                <span className="text-[10px] font-mono text-slate-500 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 px-2 py-0.5 rounded-sm">
                  {item.MaBangGia}
                </span>
                {item.TrangThai ? (
                  <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800/50 px-2 py-0.5 rounded-sm uppercase tracking-wider">Hoạt động</span>
                ) : (
                  <span className="text-[10px] font-bold text-rose-700 bg-rose-50 border border-rose-100 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800/50 px-2 py-0.5 rounded-sm uppercase tracking-wider">Tạm ngưng</span>
                )}
              </div>
              
              <h4 className="font-bold text-slate-800 dark:text-slate-100 text-base mb-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors truncate" title={item.TenDichVu}>{item.TenDichVu}</h4>
              <p className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 mb-3">
                {specialties.find(s => s.MaChuyenKhoa === item.MaChuyenKhoa)?.TenChuyenKhoa || item.MaChuyenKhoa}
              </p>
              
              <div className="mt-auto bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-sm p-4 mb-4 flex justify-center items-center">
                <p className="text-lg font-black text-rose-600 dark:text-rose-400 tracking-tight">{item.GiaKham.toLocaleString()} ₫</p>
              </div>

              <div className="flex justify-between items-center pt-3 border-t border-slate-100 dark:border-slate-800 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => toggleStatus("prices", item.MaBangGia, item.TrangThai)} className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 text-[11px] font-bold uppercase tracking-wider">
                  Đổi TT
                </button>
                <div className="flex gap-3">
                  <button onClick={() => openEditPrice(item)} className="text-emerald-600 hover:text-emerald-800 dark:text-emerald-400 dark:hover:text-emerald-300 text-[11px] font-bold flex items-center gap-1 uppercase tracking-wider">
                    <Edit className="w-3 h-3" /> Sửa
                  </button>
                  <button onClick={() => handleDelete("prices", item.MaBangGia)} className="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 text-[11px] font-bold flex items-center gap-1 uppercase tracking-wider">
                    <Trash2 className="w-3 h-3" /> Xóa
                  </button>
                </div>
              </div>
